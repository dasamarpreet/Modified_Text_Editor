...
if __name__ == '__main__':
    ...
    italic_button = tkinter.ttk.Button(tool_bar, image=italic_icon)    # replace text="" with image=bold_icon
    italic_button.grid(row=0, column=3, padx=5)

    # ======= Underline Button =================
    underline_icon = tkinter.PhotoImage(file='images/underline_image.png')
    underline_button = tkinter.ttk.Button(tool_bar, image=underline_icon)    # replace text with image=bold_icon
    ...
    def italic_text():
        text_get = tkinter.font.Font(font=text_box["font"])
        if text_get.actual()["slant"] != 'italic':
            text_box.configure(font=(font_type_now, font_size_now, "italic"))
        if text_get.actual()["slant"] == 'italic':
            text_box.configure(font=(font_type_now, font_size_now, "normal"))

    italic_button.configure(command=italic_text)

    def underline_text():
        text_get = tkinter.font.Font(font=text_box["font"])
        if text_get.actual()["underline"] == 0:
            text_box.configure(font=(font_type_now, font_size_now, "underline"))
